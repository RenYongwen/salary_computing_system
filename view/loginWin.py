...
class LoginWindow(Ui_Form, QWidget):
    ...
    def login(self, username, password):
        ...
        """
        db = DBHelp()
        count, res = db.query_super(table_name='系统用户表', column_names=['账号'], conditions=[username])
        if count == 0:
            print(count,res)
            self.login_done_signal.emit(1)
            return
        if get_md5(password) != res[0][1]:
            print(get_md5(password),res[0][1])
            self.login_done_signal.emit(11)
            return
        self.role = res[0][2]   #这部分省略了
